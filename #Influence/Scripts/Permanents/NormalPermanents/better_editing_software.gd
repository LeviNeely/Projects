...
func play() -> void:
	TurnData.common_threshold -= (TurnData.common_threshold * 0.005)
	TurnData.normal_threshold -= (TurnData.normal_threshold * 0.005)
	TurnData.uncommon_threshold -= (TurnData.uncommon_threshold * 0.005)
	TurnData.rare_threshold -= (TurnData.rare_threshold * 0.005)
...
func buy() -> void:
	...
	for permanent in TurnData.permanents:
		if permanent == null:
			TurnData.permanents[index] = "res://Scenes/Permanents/NormalPermanents/better_editing_software.tscn"
			break
		index += 1
...
func delete() -> void:
	var index: int = 0
	for permanent in TurnData.permanents:
		if permanent == "res://Scenes/Permanents/NormalPermanents/better_editing_software.tscn":
