...
var sponsor_modifier: float = 0.01
var follower_modifier: float = 0.06
var money_modifier: float = 0.18
...
func buy() -> void:
	...
	for permanent in TurnData.permanents:
		if permanent == null:
			TurnData.permanents[index] = "res://Scenes/Permanents/LegendaryPermanents/perfect_image.tscn"
			break
		index += 1
...
func delete() -> void:
	var index: int = 0
	for permanent in TurnData.permanents:
		if permanent == "res://Scenes/Permanents/LegendaryPermanents/perfect_image.tscn":
