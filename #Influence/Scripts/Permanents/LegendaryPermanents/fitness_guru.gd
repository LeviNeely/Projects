...
func play() -> void:
	if TurnData.double_permanents:
		modifier *= 2
	TurnData.viral_chance += (TurnData.viral_chance * (TurnData.sponsors / (TurnData.money / TurnData.follower_base))) * 2
...
func buy() -> void:
	...
	for permanent in TurnData.permanents:
		if permanent == null:
			TurnData.permanents[index] = "res://Scenes/Permanents/LegendaryPermanents/fitness_guru.tscn"
			break
		index += 1
...
func delete() -> void:
	var index: int = 0
	for permanent in TurnData.permanents:
		if permanent == "res://Scenes/Permanents/LegendaryPermanents/fitness_guru.tscn":
