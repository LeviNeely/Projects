...
func play() -> void:
	if TurnData.double_permanents:
		modifier *= 2
	TurnData.money += snapped((TurnData.money * modifier), 0.01)
...
func buy() -> void:
	...
	for permanent in TurnData.permanents:
		if permanent == null:
			TurnData.permanents[index] = "res://Scenes/Permanents/CommonPermanents/money_management_app.tscn"
			break
		index += 1
...
func delete() -> void:
	var index: int = 0
	for permanent in TurnData.permanents:
		if permanent == "res://Scenes/Permanents/CommonPermanents/money_management_app.tscn":
