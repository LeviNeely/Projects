...
func play() -> void:
	if TurnData.double_permanents:
		multiplier *= 2
	if TurnData.num_viral_posts > 0:
		TurnData.viral_chance += (TurnData.viral_chance * (multiplier * TurnData.num_viral_posts))
...
func buy() -> void:
	...
	for permanent in TurnData.permanents:
		if permanent == null:
			TurnData.permanents[index] = "res://Scenes/Permanents/RarePermanents/trend_setter.tscn"
			break
		index += 1
...
func delete() -> void:
	var index: int = 0
	for permanent in TurnData.permanents:
		if permanent == "res://Scenes/Permanents/RarePermanents/trend_setter.tscn":
