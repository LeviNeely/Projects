...
func play() -> void:
	if TurnData.double_permanents:
		multiplier *= 2
	TurnData.sponsor_multiplier *= multiplier
...
func buy() -> void:
	...
	for permanent in TurnData.permanents:
		if permanent == null:
			TurnData.permanents[index] = "res://Scenes/Permanents/RarePermanents/major_colab.tscn"
			break
		index += 1
...
func delete() -> void:
	var index: int = 0
	for permanent in TurnData.permanents:
		if permanent == "res://Scenes/Permanents/RarePermanents/major_colab.tscn":
